 ...
 *                  数据
 *                      传递给服务器的数据
 *              通过动态创建<script>元素来使用的,动态写入src(地址为接口地址)，服务端返回application/javascript文件
 *              可以直接执行服务端返回的代码 来实现跨域请求
 *              仅支持get请求
 *              数据通过params传递
 *          Comet
 *              服务端向客户端推送数据的技术
 *              实现comet的两种方式
 *                  长轮询
 *                      页面发送一个服务器请求，然后服务器一直保持连接打开，直到有数据可发送，发送之后浏览器关闭连接，随及发送另一个服务器请求
 *                      短轮询
 *                          页面发送一个服务器请求，然后服务器返回数据，不需要直到数据是否有效
 *                      轮询的优势在于所有浏览器都支持，因为使用XHR对象和setTimeout就能实现。控制发送时机即可
 *                  HTTP流      
 *                      在页面的整个生命周期内只使用一个HTTP连接。具体来说，就是浏览器向服务器端发送一个请求，而服务器端保持连接打开，然后周期性的向浏览器发送数据。
 *          服务器发送事件
 *              
 *          Web Sockets协议               
 *              目标是在一个单独的持久连接上提供全双工、双向通信
 *              Web Sockets使用自定义协议,非加密协议不是http://而是ws:// 机密协议也不是https:// 而是wss:// 
 *              优势
 *                  可以在客户端和服务器端发送少量的数据，而不必担心HTTP那样字节的开销
 *                  不试用同源策略
 *          SSE和Web Sockets
 *              考虑使用SSE 还是 Web Sockets时，考虑如下
 *                  是否有自由度建立和维护Web Sockets服务器？Web Sockets协议不同于HTTP，所以现有的服务器不能用于Web Sockets通信
 *                  SSE适用于HTTP通信
 *              到底是否需要双向通信
 *                  聊天室 需要双工
 *      安全
 *          CSRF（跨站伪造请求） 对未授权系统有权访问某个资源的情况
 *          防御
 *              要求以SSL连接来访问可以通过XHR请求
 *              要求每一次请求都要附加经过响应算法计算得到的验证码
 *          不起效果的防御
 *              发送POST代替GET请求 很容易改变
 *              检查URL来源 容易伪造
 *              基于cookie信息验证  容易伪造
