 ...
 *              f(undefined, 1) // 报错 暂时性死区 先访问了second但是second后定义的 所以暂时性死区问题
 * 
 *      非严格模式下 
 ...
 * 
 *      严格模式下
 *          arguments不能被重写(不能修改引用，可以改变内部的值)
 *          arguments不跟实参绑定 内部修改实参不改变arguments
 *      ES6中使用了默认参数,arguments会默认按照严格模式执行(可以被改写 但是不跟实参绑定)
 *          并不完全相同 arguments参数可以被重写 依然不与实参绑定
 *      
...
(function() {
     ...
     *      
     *      ES6中的尾调用优化
     *          仅支持严格模式下，缩减了的尾调用栈的大小(非严格模式不受影响) 满足一下条件 尾调用不再创建新的栈帧，而是清除并重用当前栈帧
