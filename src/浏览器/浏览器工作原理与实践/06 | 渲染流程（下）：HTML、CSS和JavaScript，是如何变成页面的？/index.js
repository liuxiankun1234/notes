 ...
 *  对重绘、重排、合成的理解
 *      重排：更新了元素的几何信息
 *          通过JS或者CSS修改元素的几何信息，导致浏览器会触发重新布局，解析之后的一系列子阶段
 *          重排需要更完整的渲染流水线，开销最大
 *      重绘：更新了元素的绘制属性
 *          通过JS或者CSS修改元素的背景色之类，并没有引发几何位置的改变，所以直接进入绘制阶段
 *          省去了布局和分层阶段，执行效率高一点
 *      合成：直接执行合成操作
 *          更改一个既不更新几何位置，也不更新绘制属性，渲染引擎会直接跳过，到合成执行阶段
 *          transform动画，避开了重排、重绘阶段，直接在非主线程上执行合成动画操作，效率高
 * 
 * 
 * 
 * 
 ...
 *      图层绘制（输出绘制列表 类似于canvas绘制列表）
 *          渲染引擎会对图层树中的每个图层进行绘制，会把每个图层的绘制拆分成很多小的绘制指令，将这些指令顺序生成一个待绘制列表
 *          绘制列表是一个记录绘制顺序和绘制指令的列表
 *          chrome->layer->层->Profiler可以拖动进度条重现列表的绘制过程  
 *          完成图层树的构建，渲染引擎会对图层树进行绘制        
 *      栅格化操作(raster 将图块生成位图) 图见 ./i/栅格化.png
 *          主线程生成绘制列表之后，会将绘制列表提交给合成线程进行绘制
 *          合成线程将图层划分为图块，将视口附近的的图块优先生成位图，实际生成位图的操作由栅格化线程来执行
