 ...
 *          字符限制
 *              在URL中有些字符被保留起来了，有特殊含义。还有些字符会与某些因特网网关和协议产生混淆，因此不赞成使用
 *              保留及受限的字符
 *                  这些字符将其用于保留用途之外的场合时，要在URL中对其进行编码
 *                  
 *                  字符                保留/受限
 *                  %                   保留为编码字符的转义标志
 *                  /                   保留作为路径组件中分隔路径段的分界符
 ...
 *                  ？                  保留用在查询字符串定界符使用
 *                  ;                   保留作为参数定界符使用
 *                  :                   保留作为方案、用户/口令，以及主机端口的定界符使用
 *                  $ , +               保留
 *                  @ & =               保留 在某些方案的上下文中有特殊的含义
 *                  { } | \ ^ ~ [ ]     由于各种传输Agent代理，比如各种网关的不安全处理 使用受限
 *                  < > "               不安全 这些字符在URL范围之外通常是有意义的，比如在文档中对URL自身进行定界 所以应该对其编码
 *                  0x00~0x1F，0x7F     受限 这些十六进制范围内的字符都在ASCII字符集的不可打印区间
 *                  >0x7F               受限 十六进制在此范围内的字符都不再ASCII字符集的7比特范围内
 *          另外一点说明
 *              对非安全字符进行编码是明智的 
 *              对不安全字符进行了编码，这个URL就是可在各应用程序之间共享的规范形式 
 *      方案的世界
 *          常见的方案
 *          http
 *          https
 ...
 *          telnet
 *      展望未来
 *          URL是一个强有力的工具，他可以用来命名所有的现存对象，而且可以很方便的包含一些新格式
 *          但URL并不完美。他们表示的是实际地址，一旦对应资源被移走了，URL就不再有效了
 *          URN
 *          PURL                  
 *             
 * 
 *  注意内容
 *      1、URL的查询部分或者用户输入的URL需要进行编码，防止不安全的字符在互联网中进行传输
 *      2、unicode码和ASCII字符集关系？
 *          ASCII是美国计算机最开始的字符集 
 *              用于标记字符 
 *              仅收录了美国的英文字母和一些标点符号
 *          unicode码是全面的字符集 
